...
#
# Every step1/step3 is restricted to ONE forward pass via --forward-match, so the
# call-order table at the top of the workbook and the functional-bucket summary at
# the bottom (glm52_buckets.py, which reads those same step3 files) are on the same
# scale:
#
#   SGLANG  prefill : step[EXTEND bs=3 toks=16368] (old) / toks=16332 (new)
#   ATOM    prefill : prefill[bs=3 tok=16384 ctx=[8063, 7153, 1168]]  (median-of-55)
...
# ATOM prefill forward pinned to one exact label, so both halves of the workbook
# describe the same forward.
...
for v in old new; do
  ...
  python3 trace_analysis/analyze_sglang_trace.py \
...
echo "---- ATOM prefill ----"
python3 trace_analysis/analyze_atom_trace.py --phase prefill --pick median \
...
echo "---- ATOM decode ----"
python3 trace_analysis/analyze_atom_trace.py --phase decode --pick median \
  --forward-match "$ATOM_DEC_FWD" --struct-forward-match "model.layers.0" \
  --time-trace   "$ATOM/prof_in8192_out16_conc64_p128/in8192_out16_conc64_p128-AMD-TP-0.trace.json.gz" \
  --struct-trace "$ATOM/no-cuda-graph/prof_in8192_out16_conc64_p128/in8192_out16_conc64_p128-AMD-TP-0-NoGraph.trace.json.gz" \
  --out "$DEC/sidebyside" --tag _ATOM >/dev/null

echo "################ FUNCTIONAL BUCKETS ################"
# Buckets come from the step3 workbooks above (decoder-layer scope), so there is no
# second forward-segmentation policy and this step needs no raw traces.
python3 trace_analysis/compare/glm52_buckets.py --phase prefill \
  --out "$PRE/cmp_glm52_prefill_OLDvsATOM.csv" \
...
python3 trace_analysis/compare/glm52_buckets.py --phase decode \
  --out "$DEC/cmp_glm52_decode_OLDvsATOM.csv" \
...
python3 trace_analysis/compare/side_by_side.py --align none \
  ...
  --title "GLM-5.2 prefill — call order, ONE forward per side (SGLANG step[EXTEND bs=3 toks=16368] | ATOM prefill[bs=3 tok=16384]); NOT aligned. Avg_us = one launch; LaunchCnt = launches of that call site in the forward (SGLANG: one per layer, so it equals the layers of the types in LayerType); Σ_ms = Avg_us x LaunchCnt; KernelΣ_ms/KernelCnt = that kernel name's totals in the same forward, repeated on every row sharing the name (do not sum)."
